...
details to notice here. (And if you don't get all of them now, that's fine.
...
"the current directory," which is Downloads in our case. So to get to song.mp3,
we just have to say:
...
connection between Home and Downlods and recreate it under Documents:
...
modification times shortly, but for now we will focus on another use. `touch`
creates an empty file at the path you give it if there is nothing there
already. This is the quickest way to create a file.
